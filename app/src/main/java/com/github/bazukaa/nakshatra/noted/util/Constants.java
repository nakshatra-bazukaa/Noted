package com.github.bazukaa.nakshatra.noted.util;

public class Constants {

    public static final String KEY_PREFERENCE_NAME = "notesPreference";
    public static final String KEY_GRID_STATE = "gridState";

    public static final boolean GRID_MODE = true;
    public static final boolean LIST_MODE = false;

    public static final String COLOR_1 = "#FFFFFF";
    public static final String COLOR_2 = "#FEDFA0";
    public static final String COLOR_3 = "#FF928F";
    public static final String COLOR_4 = "#8594FD";
    public static final String COLOR_5 = "#4C4C4C";

    public static final int REQUEST_CODE_STORAGE_PERMISSION = 3;
    public static final int REQUEST_CODE_SELECT_IMAGE = 4;
}
